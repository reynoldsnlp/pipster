...
def pip_help_with_python_kwargs(subcommand, cmd_obj):
...
def check_for_pipfiles(depth=3):  # `3` taken from pipenv.utils.pipfile.find_pipfile()
    pipfiles = []
    for i in range(depth):
        pipfiles.extend(glob((".." + os.sep) * i + "Pipfile"))
    if pipfiles:
        pipfiles = [os.path.abspath(f) for f in pipfiles]
        msg = "Warning: the following Pipfiles will be ignored by pipster: " + " ".join(
            pipfiles
        )
        warn(msg, stacklevel=2)


def check_if_already_loaded(requirements):
    req_targets = [get_dist_name(r) for r in requirements]
...
def get_dist_name(target: str) -> Optional[str]:
    ...
    if is_valid_url(target):
        ...
        if egg_match := re.search(r"egg=([A-Za-z0-9-_.]+)", parsed_url.fragment):
            ...
            try:
                return get_dist_name_from_filename(filename)
...
def get_dist_name_from_filename(filename):
...
def build_cmd(subcommand: str, *args, **kwargs) -> List[str]:
